...
Title: Disable Low Quality Photos GoPro (Local processing, step in Chain 1)
Last edited: 19.11.2024
User input/checks required: None
Script specifications: Minimum number of photos enabled: 1500
'''
...
if x < 1500:
    ...
    for camera in chunk.cameras:
        camera.enabled = True # Re-enables all cameras if < 1500 photos enabled
...
warnings = [] #Warning message if < 1500 photos enabled
if x <1500:
    warnings.append("WARNING: Number of photos is less than 1500 even with the lowest threshold. Manual "
