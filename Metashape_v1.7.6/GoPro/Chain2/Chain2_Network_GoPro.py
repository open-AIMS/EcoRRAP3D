...
Title: Chain 2 Network-Adapted GoPro
Last edited: 27.11.2024
User input/checks required: Input path to 'SparseCloudClean_GoPro.py' script and check network server address is correct.
...
3. Build depth maps (High quality: Downscale '2')
4. Build model (mesh)
5. Build texture
'''
...
#IN LINE 71 BELOW: task.path = "YourFilePath/SparseCloudClean_GoPro.py" # Path to the script that duplicates and renames the chunk
#endregion
...
task.path = "YourFilePath/SparseCloudClean_GoPro.py" #Check and change as required
tasks.append(task)
#endregion

#region: Build depth maps (High quality: Downscale '2')
chunk = doc.chunks[0]
