...
Title: Chain 2.1 Network-Adapted DSLR - REFERENCE MODELS ONLY
...
Script description:
Chain 2.1 Network processing. For use in the office/access to network processing for REFERENCE ONLY models.
...
3. Resize region box to 12 x 6 x 10 m (x,y,z)
4. Build low quality mesh to assess bounding box placement
'''
...
#region: Run region resize script
task = Metashape.Tasks.RunScript()
task.path = "YourFilePath/ResizeRegion.py" #Check and change as required
tasks.append(task)
#endregion

#region: Build model to inspect region placement (mesh)
